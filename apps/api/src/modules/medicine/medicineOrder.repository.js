  ...
  return {
    ...
    patientId: row.patient_id
      ? {
          ...
          uhid:
            row.patient_uhid !== null && row.patient_uhid !== undefined
              ? String(row.patient_uhid)
              : null,
        }
      : null,
  };
...
class MedicineOrderRepository {
  async findAll(filter = {}, { page = 1, limit = 10 } = {}) {
    ...
    if (filter.search) {
      ...
      conditions.push(
        sql`(COALESCE(mo.order_id, '') ILIKE ${q} OR COALESCE(p.name, '') ILIKE ${q} OR COALESCE(p.phone, '') ILIKE ${q} OR COALESCE(mo.delivery_address, '') ILIKE ${q})`,
      );
    }
    if (
      filter.patientIds &&
      Array.isArray(filter.patientIds) &&
      filter.patientIds.length > 0
    ) {
    ...
    const [totalRow] =
      await sql`SELECT count(*) FROM medicine_orders mo LEFT JOIN patients p ON mo.patient_id = p.id ${whereClause}`;
  ...
  async create(data) {
    ...
    const fullAddress = data.pinCode
      ? `${deliveryAddress} — ${data.pinCode}`
      : deliveryAddress;
    const [row] = await sql`
